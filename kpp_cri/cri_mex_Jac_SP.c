...
/* Check for the right number and size of input arguments */
 if ( nrhs != 3 ) {
   mexErrMsgTxt("cri_Jac_SP requires 3 input vectors: V(492), F(0), RCT(1472)");
 }
 mrows =  mxGetM(prhs[0]); mcols = mxGetN(prhs[0]);
 if ( ( mrows != 492 )||( mcols != 1 ) ) {
   mexPrintf("First cri_Jac_SP input argument is of size V(%d,%d).",  
               mrows, mcols);
   mexErrMsgTxt("First cri_Jac_SP input argument should be a column vector V(492,1)");
 }
 ...
 if ( (  mrows != 1472 )||( mcols != 1 ) ) {
   mexPrintf("Third cri_Jac_SP input argument is of size RCT(%d,%d).",  
               mrows, mcols);
   mexErrMsgTxt("Third cri_Jac_SP input argument should be a column vector RCT(1472,1)");
 }
 
/* Check for the right number of output arguments */
 if ( nlhs != 1 ) {
   mexErrMsgTxt("cri_Jac_SP requires 1 output column vector: JVS(4937)");
 }
 ...
 plhs[0] = mxCreateDoubleMatrix(4937,1,mxREAL);
 JVS = mxGetPr(plhs[0]);
