...
# 3. Patch cri_Global.h to prevent conflicting declarations and include parameters
if os.path.exists(global_h_path):
    print(f"Patching {global_h_path}...")
    with open(global_h_path, 'r', encoding='utf-8') as f:
        content = f.read()

    # Prepend parameter header include if not present
    if '#include "KPP/KPP_Parameters.h"' not in content and '#include "KPP_Parameters.h"' not in content:
    ...
    print(f"Error: {global_h_path} not found.")

# 4. Patch KPP_Rates.cpp
