...
/* Check for the right number and size of input arguments */
 if ( nrhs != 3 ) {
   mexErrMsgTxt("cri_Hessian requires 3 input vectors: V(442), F(0), RCT(1258)");
 }
 mrows =  mxGetM(prhs[0]); mcols = mxGetN(prhs[0]);
 if ( ( mrows != 442 )||( mcols != 1 ) ) {
   mexPrintf("First cri_Hessian input argument is of size V(%d,%d).",  
               mrows, mcols);
   mexErrMsgTxt("First cri_Hessian input argument should be a column vector V(442,1)");
 }
 ...
 if ( (  mrows != 1258 )||( mcols != 1 ) ) {
   mexPrintf("Third cri_Hessian input argument is of size RCT(%d,%d).",  
               mrows, mcols);
   mexErrMsgTxt("Third cri_Hessian input argument should be a column vector RCT(1258,1)");
 }
 
/* Check for the right number of output arguments */
 if ( nlhs != 1 ) {
   mexErrMsgTxt("cri_Hessian requires 1 output column vector: HESS(3043)");
 }
 ...
 plhs[0] = mxCreateDoubleMatrix(3043,1,mxREAL);
 HESS = mxGetPr(plhs[0]);
