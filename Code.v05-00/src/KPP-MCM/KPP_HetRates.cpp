                ...
                const double DIAMOND_RADIUS )
{
    ...
    /* HET[ind_NO3][1] += ARSL1K(DUST_SAD, DUST_RADIUS, AIRDENS, GEO_GAMMA, SQ_TEMP, SQ_NO3); */


    HET[ind_HNO3][0] += ARSL1K(NACL_SAD, NACL_RADIUS, AIRDENS, 0.1, SQ_TEMP, SQ_HNO3);

    HET[ind_N2O5][2] += ARSL1K(NACL_SAD, NACL_RADIUS, AIRDENS, 0.03, SQ_TEMP, SQ_N2O5);

    HET[ind_ClNO3][3] += ARSL1K(NACL_SAD, NACL_RADIUS, AIRDENS, 0.05, SQ_TEMP, SQ_CLONO2);

    HET[ind_HOCl][2] += ARSL1K(NACL_SAD, NACL_RADIUS, AIRDENS, 0.05, SQ_TEMP, SQ_HOCL);

    HET[ind_H2SO4][0] += ARSL1K(CACO3_SAD, CACO3_RADIUS, AIRDENS, 0.1, SQ_TEMP, SQ_H2SO4);

    HET[ind_HNO3][1] += ARSL1K(CACO3_SAD, CACO3_RADIUS, AIRDENS, 0.1, SQ_TEMP, SQ_HNO3);

    HET[ind_HCl][0] += ARSL1K(CACO3_SAD, CACO3_RADIUS, AIRDENS, 0.1, SQ_TEMP, SQ_HCL);

    HET[ind_ClNO3][4] += ARSL1K(AL2O3_SAD, AL2O3_RADIUS, AIRDENS, 0.02, SQ_TEMP, SQ_CLONO2);

    HET[ind_ClNO3][5] += ARSL1K(AL2O3_SAD, AL2O3_RADIUS, AIRDENS, 0.02, SQ_TEMP, SQ_CLONO2);

    HET[ind_N2O5][3] += ARSL1K(AL2O3_SAD, AL2O3_RADIUS, AIRDENS, 0.03, SQ_TEMP, SQ_N2O5); 

    HET[ind_HOBr][3] += ARSL1K(NACL_SAD, NACL_RADIUS, AIRDENS, 0.3, SQ_TEMP, SQ_HOBR);

    HET[ind_O3][0] += ARSL1K(DUST_SAD, DUST_RADIUS, AIRDENS, 5.0E-05, SQ_TEMP, SQ_O3); 

    HET[ind_NO3][1] += ARSL1K(DUST_SAD, DUST_RADIUS, AIRDENS, 0.01, SQ_TEMP, SQ_NO3); 

    HET[ind_N2O5][4] += ARSL1K(DIAMOND_SAD, DIAMOND_RADIUS, AIRDENS, 1.0E-04, SQ_TEMP, SQ_N2O5);
    // Include Generic GEO_SAD in O3 depletion if needed
    HET[ind_O3][0] += ARSL1K(GEO_SAD, GEO_RADIUS, AIRDENS, GEO_GAMMA, SQ_TEMP, SQ_O3);  

    HET[ind_NO2][4] += ARSL1K(DUST_SAD, DUST_RADIUS, AIRDENS, 1.0E-04, SQ_TEMP, SQ_NO2);  /* lubricant effect?, could add soot/water? */

    HET[ind_H2SO4][1] += ARSL1K(AREA[0], RADI[0], AIRDENS, 0.1, SQ_TEMP, SQ_H2SO4); /* [0] ice */

    HET[ind_H2SO4][2] += ARSL1K(AREA[3], RADI[3], AIRDENS, 0.1, SQ_TEMP, SQ_H2SO4); /* [3] */

    HET[ind_HNO3][2] += ARSL1K(AREA[0], RADI[0], AIRDENS, 0.1, SQ_TEMP, SQ_H2SO4);

    HET[ind_HNO3][3] += ARSL1K(AREA[3], RADI[3], AIRDENS, 0.1, SQ_TEMP, SQ_H2SO4);
